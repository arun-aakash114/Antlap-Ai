...
function Myprofile() {
  ...
  const [msg, setmessage] = React.useState('');
    ...
    const handleSubmit = (event) => {                  
      // NOSONAR Start
      // to get email data to pass on api
      // NOSONAR End
      ...
      if (values.currentpassword && values.newpassword && values.renewpassword) {
        ...
        try {
          axios({
            method: 'put',
            url: `${backEndDomain}/webapi/user/changepassword`,
            data: ChangepwdData,
            headers: {
    ...
    const handleChange = (prop) => (event) => {               
      // NOSONAR Start
      //to get password values to pass on api
      // NOSONAR End
  ...
  const imageupload = () => {
    try {
      axios({
        method: 'post',
        url: `${backEndDomain}/webapi/user/upload`,
        data: getimg,
        headers: {
  ...
  const handleSubmitupdate = () => {
    if (AlternateNumber.length !== 0) {
      ...
      try {
        axios({
          method: 'put',
          url: `${backEndDomain}/webapi/updateprofile`,
          data: updatedata,
          headers: {
            'Content-type': 'application/json',
            'token': jwt,
      ...
      >
        <Box sx={style}>
          ...
          <Box component="form" noValidate onSubmit={handleSubmit} >
            ...
            <Typography className="pwdcheck">{msg} {setmessage}</Typography>
            <Typography  >  {setmessage}</Typography>
