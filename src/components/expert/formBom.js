...
// NOSONAR Start
const FormBom = React.forwardRef((props, ref) => {
  ...
  const saveFunc = async () => {
    ...
    if (bomData.length !== 0) {
      ...
      let payload = {
        ...
        SMCScode: location.state.data.SMCScode || '',
      ...
      if (sessionStorage.getItem('proType') === 'Non IoT') {
        payload.ProblemCode = location.state.data.ProblemCode[0];
            ...
            >
              ...
              <Box className='card-btns'>
                ...
                <Button variant="contained" size="medium"
                  disabled={bomData.length !== 0 ? false : true}
                ...
                // NOSONAR
                // disabled={addbom}
                size="medium" color="primary" startIcon={<AddIcon />} onClick={addMoreFunc}>
                  Add
                </Button>
              </Box>
            </Box>
